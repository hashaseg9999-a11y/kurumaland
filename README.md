...
### v3: iOS / iPad 3D体験（作業ブランチ）

- メニュー・おしまい画面をCSS 3Dシーン化。空・雲・太陽・丘・道路の多層パララックスで奥行きを表現。
- 端末の傾きに応じてシーンが動くパララックス対応（iOS 13以降は初回タップ時に動作許可を要求）。
- カードグリッドに3D遷移アニメーションを追加。アクティビティ開始時・終了時の切り替えを滑らかに。
- おしまい画面に車が車庫へ戻る演出と完了メッセージを追加。
- PWAアイコンを実サイズへ最適化（192×192 / 512×512 / maskable 512×512）。合計約1.1MB → 約0.8MBに削減。
`prefers-reduced-motion` 設定時は3Dアニメーションを無効化します。
